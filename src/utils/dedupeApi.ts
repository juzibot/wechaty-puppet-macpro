...
export class DedupeApi {
  ...
  public async dedupe (
    func: (apiName: string, params?: any) => Promise<any>,
    apiName: string,
    params?: any,
    forceCall?: boolean,
  ): Promise<any> {
    ...
    if (existCall && now - existCall.timestamp < EXPIRE_TIME * 1000) {
      ...
      let result: any
      try {
        result = await this.apiQueue.execute(() => func(apiName, params))
      } catch (e) {
  ...
  private getKey (apiName: string, params?: any) {
